...
const transformOpenApiToTree = (data: any): TreeNode => {
  ...
  // Fetch only the orderAcceptChangeRQ schema
  const schemaNode = transformSchemaToNode('orderAcceptChangeRQ', definitions);

  return {
    name: 'orderAcceptChangeRQ',
    children: [schemaNode],
    isExpanded: true, // Start with the root node expanded
  };
};
...
export default function OpenAPITree({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const transformedData = transformOpenApiToTree(openapi);
      setOpenApiData(transformedData);
    };
    fetchData();
  }, []);
